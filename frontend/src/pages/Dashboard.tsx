...
const Dashboard: React.FC = () => {
  ...
  const downloadGiftContributionsPDF = (gift: Gift, giftContributions: Contribution[]) => {
    const printWindow = window.open('', '', 'height=800,width=1000');
    if (!printWindow) return;

    const eventTitle = gift.title || gift.type || 'Event';
    ...
    const htmlContent = `
      ...
      <html>
        <head>
          <title>Cash Gifts - ${eventTitle}</title>
          <style>
            body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; padding: 40px; color: #333; }
            .header { border-bottom: 2px solid #2E235C; padding-bottom: 20px; margin-bottom: 30px; }
            h1 { color: #2E235C; margin: 0; font-size: 28px; }
            .meta { color: #666; margin-top: 10px; font-size: 14px; }
            .stats { display: flex; gap: 20px; margin: 30px 0; }
            .stat-card { flex: 1; padding: 15px; background: #f8f9fa; border-radius: 8px; border: 1px solid #eee; }
            .stat-label { font-size: 12px; color: #666; text-transform: uppercase; margin-bottom: 5px; }
            .stat-value { font-size: 20px; font-weight: bold; color: #2E235C; }
            table { width: 100%; border-collapse: collapse; margin-top: 20px; }
            th { background-color: #2E235C; color: white; padding: 12px; text-align: left; font-size: 14px; }
            ...
            @media print {
              body { padding: 20px; }
              .stat-card { background: #f8f9fa !important; -webkit-print-color-adjust: exact; }
            }
          </style>
        </head>
        <body>
          <div class="header">
            <h1>Cash Gifts Received: ${eventTitle}</h1>
          ...
          <div class="stats">
            <div class="stat-card">
              ...
              <div class="stat-value">₦${totalAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
            </div>
            <div class="stat-card">
              <div class="stat-label">Total Gifters</div>
              <div class="stat-value">${giftContributions.length}</div>
            </div>
          </div>
          ...
          <table>
            ...
            <tbody>
              ${giftContributions.map(c => `
                <tr>
                  <td>${c.contributorName || 'Anonymous'}</td>
                  <td>${c.contributorEmail || '-'}</td>
                  <td class="amount">₦${Number(c.amount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
          ...
          <div class="footer">
            Generated by BeThere Experience
          </div>
        </body>
      </html>
    `;
  ...
  const [asoebiBrideDescription, setAsoebiBrideDescription] = useState('');
  const [asoebiGroomDescription, setAsoebiGroomDescription] = useState('');
  const [asoebiBrideMenDescription, setAsoebiBrideMenDescription] = useState('');
  ...
  const handleEditGift = (gift: Gift) => {
    ...
    setAsoebiBrideMenPrice(g.asoebiBrideMenPrice ? g.asoebiBrideMenPrice.toString() : '');
    setAsoebiBrideWomenPrice(g.asoebiBrideWomenPrice ? g.asoebiBrideWomenPrice.toString() : '');
    setAsoebiGroomMenPrice(g.asoebiGroomMenPrice ? g.asoebiGroomMenPrice.toString() : '');
    setAsoebiGroomWomenPrice(g.asoebiGroomWomenPrice ? g.asoebiGroomWomenPrice.toString() : '');
    setAsoebiBrideDescription(g.asoebiBrideDescription || '');
    setAsoebiGroomDescription(g.asoebiGroomDescription || '');
    setAsoebiBrideMenDescription(g.asoebiBrideMenDescription || '');
  ...
  const handleCreateGift = async (e: React.FormEvent) => {
    ...
    if (isSellingAsoebi) {
      ...
      if (type === 'wedding') {
        ...
        if (asoebiGroomMenPrice) formData.append('asoebiGroomMenPrice', asoebiGroomMenPrice);
        if (asoebiGroomWomenPrice) formData.append('asoebiGroomWomenPrice', asoebiGroomWomenPrice);
        if (asoebiBrideDescription) formData.append('asoebiBrideDescription', asoebiBrideDescription);
        if (asoebiGroomDescription) formData.append('asoebiGroomDescription', asoebiGroomDescription);
        if (asoebiBrideMenDescription) formData.append('asoebiBrideMenDescription', asoebiBrideMenDescription);
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAsoebiPriceMen('');
        setAsoebiPriceWomen('');
        setAsoebiBrideMenPrice('');
        setAsoebiBrideWomenPrice('');
        setAsoebiGroomMenPrice('');
        setAsoebiGroomWomenPrice('');
        setAsoebiBrideDescription('');
        setAsoebiGroomDescription('');
        setAsoebiBrideMenDescription('');
...
/>
      ...
      <Dialog open={showCustomMessage} onOpenChange={setShowCustomMessage}>
        <DialogContent className="w-[90vw] sm:w-full sm:max-w-lg max-h-[80vh] flex flex-col rounded-2xl border-0 shadow-2xl p-0 overflow-hidden bg-white">
          <div className="bg-gradient-to-r from-purple-600/10 to-purple-700/10 p-6 shrink-0">
            <DialogHeader>
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm mt-2">
                    Personalize your invitation message before sharing
                  </p>
                </div>
              </div>
            </DialogHeader>
          ...
          <div className="p-6 space-y-4 overflow-y-auto">
            <div className="space-y-2">
              <Label htmlFor="custom-message" className="text-sm font-medium text-gray-700">
              ...
              />
              <p className="text-xs text-gray-500">
                You can edit this message to make it more personal. The event link will be included automatically.
              </p>
            </div>
            ...
            <div className="flex gap-3 pt-4">
              ...
              >
                <img src="/whatsapp.png" alt="WhatsApp" className="w-4 h-4 mr-2 filter drop-shadow-lg brightness-110 contrast-125" />
                Send via WhatsApp
              </Button>
            </div>
          </div>
              ...
              <Button
                ...
                onClick={() => {
                  const giftContributions = contributions.filter((c) => {
                    return Number(selectedGiftForTable.id) === Number(c.giftId) && Number(c.amount) > 0;
                  });
                  downloadGiftContributionsPDF(selectedGiftForTable, giftContributions);
                }}
                className="hidden sm:flex items-center gap-2 border-[#2E235C] text-[#2E235C] hover:bg-[#2E235C] hover:text-white"
              >
                ...
                <>
                  ...
                  <div className="md:hidden space-y-3">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => downloadGiftContributionsPDF(selectedGiftForTable, giftContributions)}
                      className="w-full mb-2 flex items-center justify-center gap-2 border-[#2E235C] text-[#2E235C]"
                    >
                      <Download className="w-4 h-4" />
                      Download Gift PDF
                    </Button>
      ...
      <GuidedTour
        steps={[
          ...
          },
          {
            id: 'my-events',
            title: 'My Events',
            content: 'Create and manage all your events. Track RSVPs, Set Asoebi Price, and collect cash gifts for your event in one place.',
            target: 'body',
            position: 'center',
          ...
          },
          {
            id: 'rsvp-manager',
            title: 'RSVP Manager',
            content: 'Build and manage your guest lists. Upload guest lists via spreadsheet or manual entry. Track RSVPs. Manage guest attendance and Table seating.',
            target: 'body',
            position: 'center',
          ...
          },
          {
            id: 'invite-earn',
            title: 'Invite & Earn',
            content: 'Invite friends to join BeThere Experience and earn rewards. Grow your network and benefit from referrals.',
            target: 'body',
            position: 'center',
          ...
          },
          {
            ...
            content: 'Easily withdraw your earnings to your bank account. Secure and fast withdrawals when you need your funds.',
            target: 'body',
            position: 'center',
